...
# 2.12, PyTorch Datasets and Loaders
import torch
...
class NumberSumDataset(Dataset):
    ...
    def __getitem__(self, index):
        #print(f'num1: {index // len(self.numbers)}.')
        #print(f'num2: {index % len(self.numbers)}.')
...
tensor = model(torch.tensor([3.0, 7.0]))
print(f'test tensor: {tensor}.')
# tensor([10.1067], grad_fn=<AddBackward0>)

tensor = model(torch.tensor([2.0, 1.0]))
print(f'test tensor: {tensor}.')
tensor = model(torch.tensor([8.0, 7.0]))
print(f'test tensor: {tensor}.')
